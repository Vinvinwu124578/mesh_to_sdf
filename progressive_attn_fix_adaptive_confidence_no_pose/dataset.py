...
from tactistruct.data.dataset import _sample_indices, _sample_query_indices


def sample_rows_with_indices(num_rows: int, count: int, rng: np.random.Generator) -> np.ndarray:
    if num_rows <= 0:
        raise ValueError("num_rows must be positive.")
    if num_rows >= count:
        return rng.choice(num_rows, size=count, replace=False).astype(np.int64)
    extra = rng.choice(num_rows, size=count - num_rows, replace=True).astype(np.int64)
    return np.concatenate([np.arange(num_rows, dtype=np.int64), extra], axis=0)


def flatten_structured_view_touch_features(payload, view_index: int) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    patch_normals_view = np.asarray(payload["patch_point_normals"], dtype=np.float32)[view_index]
    patch_conf_view = np.asarray(payload["patch_point_confidence"], dtype=np.float32)[view_index]
    patch_band_view = np.asarray(payload["patch_point_band_width"], dtype=np.float32)[view_index]
    ...
    for finger_index in range(num_fingers):
        for round_index in range(num_rounds):
            ...
            points = patch_points_view[finger_index, round_index][:kept_count]
            normals = patch_normals_view[finger_index, round_index][:kept_count]
            confidence = patch_conf_view[finger_index, round_index][:kept_count].reshape(-1, 1)
            band_width = patch_band_view[finger_index, round_index][:kept_count]
            flat_features.append(np.concatenate([points, normals, confidence], axis=-1))
            flat_conf.append(confidence[:, 0])
            flat_band.append(band_width)
    ...
    return (
        np.concatenate(flat_features, axis=0).astype(np.float32),
        np.concatenate(flat_conf, axis=0).astype(np.float32),
        np.concatenate(flat_band, axis=0).astype(np.float32),
    )


class AdaptiveConfidenceProgressiveDataset(Dataset):
    def __init__(
        self,
        root: str,
        split: str = ".",
        ...
        if self.cache_in_memory:
            ...
            self.cached_bytes = int(
                sum(array.nbytes for payload in self.memory_cache for array in payload.values() if isinstance(array, np.ndarray))
            )
    ...
    def _make_rng(self, index: int) -> np.random.Generator:
        return np.random.default_rng() if self.dynamic_sampling else np.random.default_rng(self.seed + index)
    ...
    def _cache_keys_for_payload(self, payload) -> list[str]:
        keys = {
            ...
            "patch_points",
            "patch_point_normals",
            "patch_point_confidence",
            "patch_point_band_width",
            "patch_mask",
            "patch_counts",
        }
    ...
    def _sample_touch_points(
        ...
        for view_index in selected_views:
            touch_features, touch_confidence, touch_band_width = flatten_structured_view_touch_features(payload, view_index)
            if len(touch_features) > 0:
                raw_features.append(touch_features)
                raw_conf.append(touch_confidence)
                raw_band.append(touch_band_width)
        if not raw_features:
            raise ValueError("Could not load any tactile points for the selected structured views.")

        merged_features = np.concatenate(raw_features, axis=0).astype(np.float32)
        merged_conf = np.concatenate(raw_conf, axis=0).astype(np.float32)
        merged_band = np.concatenate(raw_band, axis=0).astype(np.float32)

        effective_count = int(self.num_touch_points) if self.min_touch_points >= self.num_touch_points else int(
            rng.integers(self.min_touch_points, self.num_touch_points + 1)
        )
        effective_indices = sample_rows_with_indices(len(merged_features), effective_count, rng)
        effective_features = merged_features[effective_indices]
        effective_conf = merged_conf[effective_indices]
        effective_band = merged_band[effective_indices]

        final_indices = sample_rows_with_indices(len(effective_features), self.num_touch_points, rng)
        return (
            effective_features[final_indices].astype(np.float32),
            effective_conf[final_indices].astype(np.float32),
            effective_band[final_indices].astype(np.float32),
            effective_count,
            len(selected_views),
        )
    ...
    def __getitem__(self, index: int) -> dict[str, torch.Tensor]:
        ...
        if payload is None:
            with np.load(self.files[file_index]) as loaded:
                payload = {key: np.asarray(loaded[key]) for key in self._cache_keys_for_payload(loaded) if key in loaded}
        ...
        touch_points, touch_confidence, touch_band_width, effective_count, merged_view_count = self._sample_touch_points(
            payload,
            selected_views,
            rng,
        )
        ...
        sample: dict[str, torch.Tensor] = {
            ...
            "touch_point_confidence": torch.from_numpy(touch_confidence.astype(np.float32)),
            "touch_point_band_width": torch.from_numpy(touch_band_width.astype(np.float32)),
        ...
        if "surface_normals" in payload:
            sample["surface_normals"] = torch.from_numpy(np.asarray(payload["surface_normals"], dtype=np.float32)[surface_indices])
        return sample


def load_adaptive_touch_points_from_file(
    touch_file: str | Path,
    touch_view_indices: list[int] | None = None,
) -> tuple[torch.Tensor, torch.Tensor, dict[str, int | str | None]]:
    ...
    with np.load(touch_file) as payload:
        ...
        for view_index in selected_indices:
            touch_features, _, touch_band_width = flatten_structured_view_touch_features(payload, view_index)
            merged_features.append(touch_features)
            merged_band.append(touch_band_width)
        merged_touch_points = np.concatenate(merged_features, axis=0).astype(np.float32)
        merged_band_width = np.concatenate(merged_band, axis=0).astype(np.float32)
        metadata = {
            "touch_key": "adaptive_patch_points_flattened",
        ...
        return (
            torch.from_numpy(merged_touch_points).unsqueeze(0),
            torch.from_numpy(merged_band_width).unsqueeze(0),
            metadata,
        )


__all__ = [
    "AdaptiveConfidenceProgressiveDataset",
    "flatten_structured_view_touch_features",
    "load_adaptive_touch_points_from_file",
]
