...
from dataset_legacy_progressive import StructuredLegacyProgressiveDataset
...
DEFAULT_OUTPUT_DIR = "outputs/progressive_attn_fix_legacy_from_structured"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Train a legacy progressive_attn_fix-style model on structured MuJoCo coverage-aware data "
            "without modifying the original progressive_attn_fix package."
        )
    )
...
def maybe_init_wandb(config: dict, output_dir: Path):
    ...
    return wandb.init(
        project=wandb_cfg.get("project", "progressive-attn-fix-legacy-from-structured"),
...
def build_dataset_cfg(
    ...
    return {
        "name": "structured_legacy_progressive",
...
def build_config(args: argparse.Namespace) -> tuple[dict, dict[str, list[str]]]:
    ...
    config = {
        ...
        "wandb": {
            ...
            "project": "progressive-attn-fix-legacy-from-structured",
            "name": output_dir.name,
            "mode": "online",
            "tags": ["progressive-attn-fix", "legacy-architecture", "structured-mujoco", "no-pose"],
        },
    }
    if val_files:
...
def build_dataset(cfg: dict) -> StructuredLegacyProgressiveDataset:
    params = {key: value for key, value in cfg.items() if key != "name"}
    return StructuredLegacyProgressiveDataset(**params)
    ...
    for batch_index, batch in enumerate(iterator, start=1):
        ...
        with torch.set_grad_enabled(train_mode):
            ...
            with autocast_context:
                outputs = model(
                    batch,
                    apply_augmentation=train_mode and apply_touch_augmentation,
                )
                losses = compute_progressive_attn_losses(
                    batch,
                    outputs,
                    loss_cfg,
                    include_eikonal=True,
                )
                total = losses["total"]
...
def main() -> None:
    ...
    config_path = save_json(output_dir / "progressive_attn_fix_legacy_config.json", config)
